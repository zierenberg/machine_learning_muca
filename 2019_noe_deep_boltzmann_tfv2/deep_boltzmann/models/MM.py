...
def dist_tf(x1, x2):
    d = x2-x1
    d2 = tf.reduce_sum(input_tensor=d*d, axis=2)
    return tf.sqrt(d2)
...
def angle_tf(x1, x2, x3):
    ba = x1 - x2
    ba /= tf.norm(tensor=ba, axis=2, keepdims=True)
    bc = x3 - x2
    bc /= tf.norm(tensor=bc, axis=2, keepdims=True)
    cosine_angle = tf.reduce_sum(input_tensor=ba*bc, axis=2)
...
def torsion_tf(x1, x2, x3, x4, degrees=True):
    ...
    # normalize b1 so that it does not influence magnitude of vector
    # rejections that come next
    b1 /= tf.norm(tensor=b1, axis=2, keepdims=True)
    ...
    # w = projection of b2 onto plane perpendicular to b1
    #   = b2 minus component that aligns with b1
    v = b0 - tf.reduce_sum(input_tensor=b0*b1, axis=2, keepdims=True) * b1
    w = b2 - tf.reduce_sum(input_tensor=b2*b1, axis=2, keepdims=True) * b1

    # angle between v and w in a plane is the torsion angle
    # v and w may not be normalized but that's fine since tan is y/x
    x = tf.reduce_sum(input_tensor=v*w, axis=2)
    b1xv = tf.linalg.cross(b1, v)
    y = tf.reduce_sum(input_tensor=b1xv*w, axis=2)
    if degrees:
...
class MM(object):
    ...
    def energy_tf(self, x):
        ...
        if self.toppar.bond_indices.size > 0:
            E += tf.reduce_sum(input_tensor=self.toppar.bond_k * ((self.bondlengths_tf(x) - self.toppar.bond_d0)**2), axis=1)
        if self.toppar.angle_indices.size > 0:
            da = (self.angles_tf(x) - self.toppar.angle_a0) * 0.017453  # in radians
            E += tf.reduce_sum(input_tensor=self.toppar.angle_k * (da**2), axis=1)
        if self.toppar.torsion_indices.size > 0:
            tor = np.float32(np.pi / 180.0) * self.torsions_tf(x)
            phase = np.float32(np.pi / 180.0) * self.toppar.torsion_phase
            E += tf.reduce_sum(input_tensor=self.toppar.torsion_k * tf.cos(self.toppar.torsion_n*tor - phase), axis=1)
        if self.toppar.nonbonded_indices.size > 0:
            dist = self.nb_dists_tf(x)
            f6 = (self.toppar.nonbonded_sigma / dist) ** 6
            E += tf.reduce_sum(input_tensor=self.toppar.nonbonded_epsilon * (f6**2 - 2*f6), axis=1)
        if self.toppar.nonbonded14_indices.size > 0:
            dist = self.nb14_dists_tf(x)
            f6 = (self.toppar.nonbonded14_sigma / dist) ** 6
            E += tf.reduce_sum(input_tensor=self.toppar.nonbonded14_epsilon / 1.2 * (f6**2 - 2*f6), axis=1)
