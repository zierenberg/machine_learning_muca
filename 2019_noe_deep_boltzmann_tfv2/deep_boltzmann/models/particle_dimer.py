...
class ParticleDimer(object):
    ...
    def LJ_energy_tf(self, x):
        ...
        n = tf.shape(input=xcomp)[1]
        Xcomp = tf.tile(tf.expand_dims(xcomp, 2), [1, 1, n])
        Ycomp = tf.tile(tf.expand_dims(ycomp, 2), [1, 1, n])
        Dx = Xcomp - tf.transpose(a=Xcomp, perm=[0, 2, 1])
        Dy = Ycomp - tf.transpose(a=Ycomp, perm=[0, 2, 1])
        D2 = Dx**2 + Dy**2
        ...
        D2rel = D2rel * mmatrix
        # energy
        E = 0.5*self.params['eps']*tf.reduce_sum(input_tensor=D2rel**6, axis=(1, 2))  # do 1/2 because we have double-counted each interaction
        return E
    ...
    def box_energy_tf(self, x):
        ...
        E = 0.0
        d_left = -(xcomp + self.params['box_halfsize'])
        E += tf.reduce_sum(input_tensor=(tf.sign(d_left) + 1) * self.params['box_k'] * d_left**2, axis=1)
        d_right = (xcomp - self.params['box_halfsize'])
        E += tf.reduce_sum(input_tensor=(tf.sign(d_right) + 1) * self.params['box_k'] * d_right**2, axis=1)
        d_down = -(ycomp + self.params['box_halfsize'])
        E += tf.reduce_sum(input_tensor=(tf.sign(d_down) + 1) * self.params['box_k'] * d_down**2, axis=1)
        d_up = (ycomp - self.params['box_halfsize'])
        E += tf.reduce_sum(input_tensor=(tf.sign(d_up) + 1) * self.params['box_k'] * d_up**2, axis=1)
        return E
    ...
    def grid_energy_tf(self, x):
        d2 = (x - self.grid)**2
        E = tf.reduce_sum(input_tensor=self.params['grid_k'] * (self.params['rm']**2 * d2) ** 6, axis=1)
        return E
