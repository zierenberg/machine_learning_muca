...
def _logreg_tf(x, a=0.001):
    logx = tf.compat.v1.where(x > a, tf.math.log(x), tf.math.log(a) - (a-x))
    return logx
    ...
    def _call(*args, **kwargs):
        ...
        if len(xs) > 0:
            def _grad_fun(*grad_ys):
                return transform_fn(xs, ys, tf.gradients(ys=ys, xs=xs, grad_ys=grad_ys))
            return ys, _grad_fun
        else:
            return ys
    return _call
...
def gradient_clip(op, cutoff_min, cutoff_max):
    def _grad_clip_fn(_, __, grads):
        grads_clipped = []
        for g in grads:
            norm = tf.norm(tensor=g, axis=-1, keepdims=True)
