...
class FlexibleTrainer(object):
    ...
    def loss_ML(self, y_true, y_pred):
        ...
        LL = Jxz - (0.5 / (self.std**2)) * tf.reduce_sum(input_tensor=z**2, axis=1)
        return -LL
    ...
    def loss_ML_weighted(self, y_true, y_pred):
        ...
        # weights
        Ez = tf.reduce_sum(input_tensor=z**2, axis=1)/(2.0*self.tfac)
        logW = -Ereg + Ez - Jxz
        logW = logW - tf.reduce_max(input_tensor=logW)
        weights = tf.exp(logW)
        # weighted ML
        weighted_negLL = -self.batch_size * (weights * LL) / tf.reduce_sum(input_tensor=weights)
...
class ForceMatching(object):
    ...
    def loss_force_matching(self, y_true, y_pred):
        ...
        neg_log_px = 0.5 * tf.reduce_sum(input_tensor=z_out**2, axis=1) - Jxz

        fx = -tf.gradients(ys=neg_log_px, xs=x_in)[0]
        fx_ref = -tf.gradients(ys=enerx, xs=x_in)[0]

        force_error = fx - fx_ref
        energy_error = neg_log_px - enerx
        mse = tf.reduce_mean(input_tensor=self.w_force * force_error**2, axis=1) + self.w_energy * energy_error**2
        return mse
...
class ParticleFilter(object):
    ...
    def loss_ML_weighted(self, y_true, y_pred):
        ...
        Ereg = linlogcut(E, self.high_energy, self.max_energy, tf=True)
        # weights
        Ez = tf.reduce_sum(input_tensor=z**2, axis=1)/(2.0*self.temperature)
        logW = -Ereg + Ez - Jxz
        logW = logW - tf.reduce_max(input_tensor=logW)
        weights = tf.exp(logW)
        # weighted ML
    ...
    def rc_plogp(self, rc_func, gmeans, gstd, ntemperatures=1):
        ...
        entropies = tf.reduce_sum(input_tensor=histogram * tf.math.log(histogram), axis=1)
