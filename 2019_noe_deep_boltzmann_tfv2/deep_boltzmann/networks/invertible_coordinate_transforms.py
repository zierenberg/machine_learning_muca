...
def log_det_jacobian(outputs, inputs):
    ...
    return tf.reduce_sum(input_tensor=tf.math.log(s), axis=1, keepdims=True)
...
class FixedWhiten(object):
    ...
    def connect_xz(self, x):
        ...
        if self.keepdims < self.dim:
            ...
            self.output_z = keras.layers.Lambda(lambda z: tf.concat([z, tf.random.normal([tf.shape(input=z)[0], junk_dims], stddev=1.0)], 1))(self.output_z)
        # Jacobian
        self.log_det_xz = keras.layers.Lambda(lambda x: self.jacobian_xz * keras.backend.ones((tf.shape(input=x)[0], 1)))(x)
    ...
    def connect_zx(self, z):
        ...
        # Jacobian
        self.log_det_zx = keras.layers.Lambda(lambda z: -self.jacobian_xz * keras.backend.ones((tf.shape(input=z)[0], 1)))(z)
...
def xyz2ic_tf(x, Z_indices, torsion_cut=None):
    ...
    if torsion_cut is not None:
        xtorsions = tf.compat.v1.where(xtorsions < torsion_cut, xtorsions+360, xtorsions)
    # Order ic's by atom
    if global_ic:
        iclist = [xbonds[:, 0:2], xangles[:, 0:1]]
        for i in range(Z_indices.shape[0]-3):
...
def ic2xyz(p1, p2, p3, d14, a124, t1234):
    ...
    n /= tf.norm(tensor=n, axis=1, keepdims=True)
    nn /= tf.norm(tensor=nn, axis=1, keepdims=True)

    n *= -tf.sin(t1234)
    nn *= tf.cos(t1234)

    v3 = n + nn
    v3 /= tf.norm(tensor=v3, axis=1, keepdims=True)
    v3 *= d14 * tf.sin(a124)

    v1 /= tf.norm(tensor=v1, axis=1, keepdims=True)
    v1 *= d14 * tf.cos(a124)
...
def ics2xyz_global(ics, Z_indices):
    ...
    batchsize = tf.shape(input=ics)[0]
...
def log_det_jac_lists(ys, xs):
    ...
    log_det = tf.reduce_sum(input_tensor=log_det, axis=0)
...
def ics2xyz_local_log_det_jac_batchexpand(ics, Z_indices, index2zorder, xyz, eps=1e-10):
    ...
    log_det_jac = tf.reduce_sum(input_tensor=log_det_jac, axis=1)
...
def ics2xyz_local_log_det_jac_decomposed(all_ics, all_Z_indices, cartesian_xyz, index2order, eps=1e-10):
    ...
    """
    batchsize = tf.shape(input=all_ics)[0]
...
def xyz2ic_log_det_jac(x, Z_indices, eps=1e-10):
    ...
    if global_transform:
        ...
        log_det_jac_ = tf.reduce_sum(input_tensor=log_det_jac_, axis=-1)
...
class InternalCoordinatesTransformation(object):
    ...
    def connect_xz(self, x):
        ...
        self.output_z = keras.layers.Lambda(
                lambda z: tf.concat([z, 0. * tf.random.normal([tf.shape(input=z)[0], junk_dims], stddev=1.)], 1))(self.output_z_only)
...
class MixedCoordinatesTransformation(InternalCoordinatesTransformation):
    ...
    def z2x(self, z):
        ...
        batchsize = tf.shape(input=z)[0]
        xyz = tf.reshape(x_cart, (batchsize, self.cart_atom_indices.size, 3))

        def _angle_loss(angle):
            positive_loss = tf.reduce_sum(input_tensor=tf.compat.v1.where(
                angle > 180, angle - 180, tf.zeros_like(angle)) ** 2, axis=-1)
            negative_loss = tf.reduce_sum(input_tensor=tf.compat.v1.where(
                angle < -180, angle + 180, tf.zeros_like(angle)) ** 2, axis=-1)
